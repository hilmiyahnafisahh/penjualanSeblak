...
// tambahan untuk menggunakan apriori association rules
use Phpml\Association\Apriori;

class KeranjangController extends Controller
{
    // tambahan untuk mendapatkan rekomendasi produk menggunakan algoritma apriori
    private function getRekomendasiProduk($keranjangItems)
    {
        // =========================
        // 1. Ambil histori transaksi
        // =========================
        $transaksi = DB::table('penjualan_barang')
            ->join('penjualan', 'penjualan.id', '=', 'penjualan_barang.penjualan_id')
            ->join('pembayaran', 'penjualan.id', '=', 'pembayaran.penjualan_id')
            ->where('pembayaran.status_code', 200) // hanya transaksi sukses
            ->select('penjualan.id as transaksi_id', 'penjualan_barang.barang_id')
            ->get();

        // =========================
        // 2. Bentuk dataset transaksi
        // =========================
        $dataset = [];

        foreach ($transaksi as $t) {
            $dataset[$t->transaksi_id][] = (string) $t->barang_id;
        }

        $samples = array_values($dataset);
        ...
        // 3. Apriori
        // =========================
        $associator = new Apriori(0.1, 0.1); // support, confidence
        $associator->train($samples, []);

        $rules = $associator->getRules();

        // =========================
        // 4. Ambil barang di keranjang
        // =========================
        $cartItems = collect($keranjangItems)->pluck('barang_id')->map(fn($i) => (string)$i)->toArray();

        $rekomendasiIds = [];

        foreach ($rules as $rule) {
            $antecedent = $rule['antecedent']; // jika
            $consequent = $rule['consequent']; // maka

            // cek apakah keranjang mengandung antecedent
            if (count(array_intersect($antecedent, $cartItems)) > 0) {
        ...
        // =========================
        if (empty($rekomendasiIds)) return [];

        return DB::table('barang')
            ->whereIn('id', $rekomendasiIds)
            ->limit(4)
            ->get();
    }
        ...
        if(isset($outputjson)){
            ...
            if($outputjson['status_code']=='404' or in_array($outputjson['transaction_status'], ['expire', 'cancel', 'deny'])){
                ...
                if($ttl>0){
                    ...
                    // modifikasi sehingga get rekomendasi produk dulu baru dikirim ke halaman view
                    $rekomendasi = $this->getRekomendasiProduk($barang);
                    // dd($rekomendasi);
    ...
    public function hapus($barang_id)
    {
        try{
            ...
            $sql = "DELETE FROM penjualan_barang WHERE barang_id = ? AND penjualan_id IN (SELECT penjualan.id FROM penjualan join pembayaran on (penjualan.id=pembayaran.penjualan_id) WHERE penjualan.pembeli_id = ? AND ((pembayaran.gross_amount = 0) or (pembayaran.jenis_pembayaran='pg' and pembayaran.status_code<>'200')))";
            $deleted = DB::delete($sql, [$barang_id,$id_pembeli]);
            
            // cek jika sudah kosong maka hapus data penjualan juga dan pembayaran juga
            // =========================
            // CEK & HAPUS PENJUALAN YANG SUDAH KOSONG
            // =========================
            $penjualanKosong = DB::table('penjualan')
                ->leftJoin('penjualan_barang', 'penjualan.id', '=', 'penjualan_barang.penjualan_id')
                ->join('pembayaran', 'penjualan.id', '=', 'pembayaran.penjualan_id')
                ->where('penjualan.pembeli_id', $id_pembeli)
                ->whereNull('penjualan_barang.id') // tidak ada item
                ->where(function($query) {
                    $query->where('pembayaran.gross_amount', 0)
                        ->orWhere(function($q) {
                            $q->where('pembayaran.status_code', '!=', 200)
            ...
            foreach ($penjualanKosong as $p) {
                DB::table('pembayaran')->where('penjualan_id', $p->id)->delete();
                DB::table('penjualan')->where('id', $p->id)->delete();
            }

            // query penjualan yang belum terbayar
            ...
            // Safety: Jika keranjang kosong, set total ke 0
            $totalBaru = $tagihan->total ?? 0;

            if ($penjualan) {
                DB::table('penjualan')
                    ->where('id', $penjualan->id)
                    ->update(['tagihan' => $totalBaru]);
            }
            ...
            return response()->json(['success' => true, 'message' => 'Produk berhasil dihapus', 'total' => $totalBaru, 'jmlbarangdibeli'=>$jmlbarangdibeli[0]->total ?? 0]);
        }catch (\Exception $e) {
            // Jika ada error (DB error dsb), kirim sebagai JSON agar JS tidak kaget terima HTML
            return response()->json([
                'success' => false,
